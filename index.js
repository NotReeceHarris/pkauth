...
class pkAuth {
  ...
  encryptPrivateKey = async (privateKey, passphrase) => { // An async function that encrypts a private key with a passphrase
    try {
      ...
      return (await encryptKey).armor() // Wait for the key to encrypt and return the encrypted key
    } catch (error) {
      throw error(error)
    }
  }
  ...
  decryptPrivateKey = async (encryptedPrivateKey, passphrase) => { // An async function that decrypts an encrypted private key with a passphrase
    try {
      ...
      return (await decryptKey).armor() // Wait for the key to decrypt and return the decrypted key
    } catch (error) {
      throw error(error)
    }
  }
